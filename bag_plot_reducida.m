...
imu2                    = select(bag, 'Topic', '/vectornav/IMU'); %/imu2
imu3                    = select(bag, 'Topic', '/vectornav/IMU'); %/imu3
...
for i=1:1
    ...
    huskyLinVel = huskyOdomVelData{1,1}.Twist.Twist.Linear.X;
    huskyAngVel = huskyOdomVelData{1,1}.Twist.Twist.Angular.Z;   
...
setMtxR(mhe4gps,diag([1 0.1 0.1 0.1 0.1 10 10 10 10]));
...
Rk                          = diag([0.000986960440108936, 0.053125, 0.053125]); %1
...
for i = 1:num_msgs-data_init+1
    ...
    % updateMeasurement(mheLoss, [correctedAtt(i); pos]);
    % updateInput(mheLoss, velocities(:, i));
    % solve(mheLoss);
    % q_k = mheLoss.qk;
...
disp('Diferencia de error promedio:')
disp(mean(dif_error(50:end)))
...
xlabel('Iteración')
ylabel('Tiempo [s]')
title('Tiempo de ejecucion por iteración')
grid on