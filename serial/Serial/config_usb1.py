...
def init_serial(baudios):
    try:
        ser = serial.Serial(PORT, baudios, timeout=TIMEOUT)
        print(f"Conexión establecida en {PORT} a {baudios} baudios.")
        return ser
...
def main():
    BAUDRATE = 9600
    ser = init_serial(BAUDRATE)
    
    rate = 5 #input("Introduce la nueva frecuencia de actualización (en Hz, ej. 5): ")
    change_update_rate(ser, int(rate))
    baudrate = 115200 #input("Introduce el nuevo baudrate (ej. 9600, 38400, 57600, 115200): ")
