...
Rmhe = diag([gps_noise_factor, gps_noise_factor, gyro_noise_factor]);  % Measurement cost (greater means less confidence)
Qmhe = diag([10, 10, 1] * 0.01);            % Process noise cost
Gmhe = diag([1, 1, 0.1] * 0.0002);         % Slack cost (greater means faster but less accurate)(for 0.0001 factor the error is equal to EKF)
Pmhe = diag([gps_noise_factor, gps_noise_factor, gyro_noise_factor]);  % Arrival cost
...
for k = 1:Nh
    ...
    % GPS measurements
    optiMHE.subject_to(qMHE(:, k) + vMHE(:, k) == ([y_meas(1:2, k); y_meas(4 * 2 + 1, k)] - R * 0.5 * [lg; -wg; 0] + slackGps1(:, k))*alphaMHE(1,k)+([y_meas(3:4, k); y_meas(4 * 2 + 1, k)] - R * 0.5 * [lg; wg; 0] + slackGps2(:, k))*alphaMHE(2,k)+([y_meas(5:6, k); y_meas(4 * 2 + 1, k)] - R * 0.5 * [-lg; wg; 0] + slackGps3(:, k))*alphaMHE(3,k)+([y_meas(7:8, k); y_meas(4 * 2 + 1, k)] - R * 0.5 * [-lg; -wg; 0] + slackGps4(:, k))*alphaMHE(4,k) );
